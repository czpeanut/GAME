...
### ⚠️ 底圖上有的五官，替換用的圖層一定要蓋得住

學姊的 `eyes/closed.png` **不完全是繪師的圖**，這是目前唯一還需要補的東西。

繪師給的閉眼圖只有睫毛那條線——那是假設底下什麼都沒有才成立的。但臉的圖層上
**已經畫了眼白**，所以只疊一條睫毛線上去，眨眼時會看到兩顆翻白的眼睛，實際跑
起來非常明顯。

現在的 `closed.png` 是「睫毛線 ＋ 一塊只填住眼白的眼皮膚色補丁」，補丁顏色取自
眼睛周圍皮膚的中位數。看起來可以，但那是補的不是畫的。

**要修乾淨只需要一張圖：一張不含眼睛的臉。** 有了它，繪師原本的閉眼圖就直接能
用，補丁整個丟掉。嘴巴沒有這個問題——三張嘴型都蓋得住臉上原本那條嘴線。
