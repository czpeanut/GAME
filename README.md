...
**部件名稱不是固定清單**，rig 想叫什麼就叫什麼——引擎只是拿部件名去組路徑
`assets/characters/<角色id>/<部件>/<變體>.png`，再按 rig 陣列的順序由後往前畫。
下面這張表是**橫向切割**那條路線的慣用名稱；如果美術給的是真正的分層，
照著分層命名會更自然（見「兩條路線」）。
...
```bash
python3 tools/split-parts.py 基底.png assets/characters/hero \
    head:0:0.25 torso:0.19:0.47 lower:0.43:1
```

（這裡的「基底」是**已經把手臂移除、並補畫好後面**的那張，不是原始完稿——
原因見下方。）
...
`assets/characters/hero/` 的 `head`/`torso`/`lower` 就是這樣切出來的，另外
`arm_l`/`arm_r`/`hair_front` 是美術另外提供的分層；`mouth` 是從三張嘴型完稿
截出來的（見下一節）；`eyes` 還沒有，所以眨眼是關著的。**沒提供的部件會自動
跳過**，可以一張一張補，程式完全不用改。

> **手臂一定要從底圖挖掉，而且要補畫後面。** 一開始用的 `全身` 完稿把手臂
> 烙在裡面（分層是額外提供的複本，不是挖掉），手臂擺動時底下那隻不動的
> 手臂就會從邊緣露出來——看起來像有兩隻手。現在用的 `基底` 是把手臂移除、
> 並且把被擋住的軀幹補畫完整的版本，所以手臂可以自由擺動，後面永遠是完整的
> 身體。實測把擺幅放大到 ±8 度（實際只有 ±3.5 度），肩膀接縫不會露出任何背景。
>
> 頭髮還是疊在自己的複本上（`hair_front` 蓋住底圖原本的頭髮），這個沒問題，
> 因為頭髮的擺動幅度小、而且露出來的是同樣的頭髮。搭配上面的重疊切法，
> 脖子和腰的接縫在 4 倍幅度下也還是乾淨的。

### 兩條路線：橫向切割 vs 真正的分層

上面講的是**從一張壓平的完稿橫向切**——沒有分層時的權宜之計，做得到呼吸和
頭部微傾，但切不出手臂、頭髮和五官，因為那些會互相遮擋。

如果美術願意給**真正的分層**，限制就全部消失，而且不需要重疊切法：每一層自己
就是完整的，動起來後面永遠有東西。目前 repo 裡的 `senpai`（學姊）就是這樣來的
——十層，一層一個部件：

| 部件 | 這層是什麼 | 動態 |
| --- | --- | --- |
| `hair_back` | 長髮（整片，在最後面） | 彈簧跟隨頭部，全身最明顯的延遲 |
| `lower` | 雙腿 | 完全不動 |
| `torso` | 軀幹＋**左手**（左手畫在這層裡） | 呼吸、微傾 |
| `skirt` | 裙子（含裙擺內裡） | 從腰擺動，最軟最慢的彈簧 |
| `bodice` | 上身衣料（合身，所以只跟著軀幹） | 跟著軀幹 |
| `arm_r` | 舉起來的右手（整隻含手掌） | 自己的擺動＋彈簧 |
| `head` | 臉 | 微傾、隨呼吸上下 |
| `eyes` | 整顆眼睛，三張：`open` / `half` / `closed` | 眨眼 |
| `mouth` | 嘴，三張：`closed` / `half` / `open` | 說話時輪替＝嘴型 |
| `hair_front` | 瀏海 | 彈簧跟隨頭部，比後髮硬 |
| `bows` | 腰側的兩個蝴蝶結 | 最快最彈的彈簧 |

**裙子是這種角色最值得分出來的一層。** 穿裙子的角色，裙擺慢半拍地跟著身體走，
比其他任何一個部件都更能讓她看起來是活的。

美術的分層常常是**白底 JPG**（匯出時把透明度丟掉了）。`tools/key-layers.py`
會把背景去掉、把所有層裁成同一個畫布，再寫成引擎要的檔案結構：

```bash
python3 tools/key-layers.py assets/characters/senpai \
    hair_back=raw/長髮.jpg lower=raw/腿.jpg torso=raw/身體.jpg \
    skirt=raw/裙子.jpg bodice=raw/上衣.jpg arm_r=raw/右手.jpg \
    head=raw/臉.jpg hair_front=raw/瀏海.jpg bows=raw/蝴蝶結.jpg \
    eyes/open=raw/睜眼.jpg eyes/half=raw/半闔.jpg \
    mouth/closed=raw/閉嘴.jpg mouth/half=raw/半開.jpg mouth/open=raw/張嘴.jpg
```

部件有多個變體時在名稱後面加 `/變體`；沒寫就是 `default`。眨眼是
`open → half → closed → half → open`，兩端各經過一格半闔眼——直接從睜開跳到閉上
在近距離看會像畫面壞掉，中間那一格只花兩個影格，但看起來才像眼皮。

去背用的是**從邊界往內的填充**，不是「比某個亮度白就挖掉」。這個差別很重要：
這個角色穿米白色洋裝、眼睛裡有白色高光，用亮度門檻會直接在洋裝和眼睛上打洞。
被畫稿包住的白進不去，所以留得下來。

⚠️ **請美術匯出 PNG 而不是 JPG。** JPG 會在邊緣產生壓縮雜訊，去背之後邊緣會
比原稿髒一點。目前這組素材實測還可以接受，但 PNG 一定更乾淨。

⚠️ **頭髮裡會「困住」背景。** 髮絲之間包住的白色，從邊界灌水灌不進去，會變成
一塊塊不透明的白斑——貼在白色網頁上看不出來，放到教室背景前就是頭髮破了好幾個
洞。所以工具有第二道處理：把**畫稿包住、而且接近純白**的區域也清掉。門檻比第一
道嚴格得多（預設 250），因為實測這個角色的米白洋裝最亮只到 237、眼睛高光只到
247，而頭髮裡困住的背景是 250 以上——兩者分得開。如果你的素材真的有大片純白，
用 `--enclosed-white` 調高或設 0 關掉。

驗收方式：十層疊回去要能還原完稿。實測這組素材疊起來跟完稿幾乎一模一樣，
而且把所有動態通道推到最大時，**人物內部只有 0.09% 的像素會露出背景**，
而且是散的、不是一條線——也就是沒有接縫問題。
...
第一版的臉圖層上畫了眼白和睫毛。繪師另外給的閉眼圖只有一條睫毛線——那是假設
底下什麼都沒有才成立的。兩者疊起來的結果是:眨眼時看到兩顆翻白的眼睛,而且底圖
那層睫毛在眼睛變化時完全不動。**不是哪一張畫錯了,是分層的分法錯了。**

現在的臉圖層**沒有眼睛也沒有嘴巴**,只有臉型、鼻子、腮紅、眉毛和耳朵。三張眼睛
和三張嘴型直接疊上去就對了,完全不需要任何補丁。

所以規則是:**任何會被動畫替換的部位,底圖上都不能先畫**。要替換眼睛,臉上就不能
有眼睛;要替換嘴巴,臉上就不能有嘴巴。眉毛沒有要替換,所以留在臉上沒問題。

### 動態幅度是「每個角色自己的」，不是全域的

`rig.js` 裡的 `BREATHE_SCALE` / `TILT_RAD` / `SWAY_RAD` 是**權重 1 的基準值**，
刻意訂得很小——因為從壓平完稿橫向切出來的角色，幅度一大接縫就露餡。

真正的分層角色沒有這個限制，所以**不要去改那三個全域常數**（會把切片角色弄壞），
改兩個地方就好：

- **動多遠** → rig 裡每個部件的權重。權重可以大於 1：學姊的 `torso` 是
  `tilt: 3`，`skirt` 是 `sway: 2.6`。
- **動多快** → 角色的 `motion` 設定（`Character` 的選項，`Stage` 會轉交給
  `PortraitMotion`）。學姊用 `breathePeriod: 3.2 / tiltPeriod: 4.3 /
  swayPeriod: 3.1`，比預設快不少；三個週期刻意不成倍數，才不會看出循環。
...
學姊在這個幅度下連跑 45 影格，**人物內部沒有任何一個像素露出背景**。切片角色
做不到這件事，這就是分層素材真正買到的東西。

### 子部件跟著父部件走，不會自己算一次

呼吸讓軀幹垂直拉伸,頭部騎在上面被抬起來——但頭不該被拉長,所以它要把繼承來的
縮放除回去。問題在於**每個部件是繞著自己的關節點除回去的**,而關節點離腰越遠,
除回去時位移就越大。

頭髮的關節在頭頂(0.035),頭的關節在脖子(0.205),結果是:
...
頭髮比它黏著的那顆頭多跑了一倍——看起來就是一頂假髮在頭上滑動。

修法是 `parentScaleY` 改成累積**祖先實際套用的縮放**,而不是它們名義上的
`scaleY`。頭已經把縮放除掉了,所以掛在頭下面的頭髮繼承到的是 1,不會再除第二次,
就乖乖跟著頭走。修正後頭髮、眼睛、嘴巴的位移都跟頭一樣是 5.4 px;手臂因為肩膀
比脖子低,是 4.0 px;裙子和蝴蝶結掛在腰上,位移 0。全部符合解剖。

`test/rig.test.mjs` 有這條的回歸測試。

### 部件圖是整張畫布，但引擎只畫有東西的那一塊

「每個部件都是同樣大小的整張畫布」這個慣例的好處是部件天生對齊，不需要任何額外
的位置資料。代價是**每一格都在畫 11 個全尺寸矩形**——一張 32px 寬的嘴，跟整條
裙子一樣貴。

所以 `PortraitRenderer` 會在圖片載入時量一次它實際有像素的範圍，之後只畫那一塊。
素材格式不用改，也不需要額外的設定檔。
...
同一格畫面用兩種方式各畫一次，兩百萬個像素裡只有 8 個差超過 24 階（縮放取樣的
捨入），所以是純粹的速度改善。沒有這個，人物一放大 fps 就從 60 掉到 45。
