...
`eyes` / `mouth` 不必單獨畫一張透明底的五官。只要給**同一張立繪的不同版本**
——嘴閉合／半開／張開三張，或眼睛睜開／閉上兩張——工具會自己找出「哪裡變了」，
把那塊切成部件：
...
做法是逐像素比對所有版本，取出有差異的區域，再從每個版本把那塊方框切下來。
這樣切出來的接縫**不是「幾乎看不出來」，而是數學上完全沒有接縫**：引擎把
`mouth` 疊在 `head` 上面，而方框外每張圖都是一模一樣的像素，所以方框邊緣兩
側的顏色本來就相同，不需要羽化。實測「`head`（閉嘴版）＋ `mouth/open`」疊
起來跟原本的張嘴完稿 RGB 完全相同，只有 16 個原本半透明（alpha 252~254）
的像素被補滿成 255，肉眼不可能看見。

⚠️ **唯一的前提：三張圖除了嘴巴以外必須逐像素對齊。** 必須是同一個檔案只換
掉嘴巴那層再匯出，不能是三次各自生成／各自重畫的結果——那樣整個人都會位移，
截出來的方框會跟著臉一起跳。工具會先把差異區域列出來並擋下來：實測把其中一
張只平移 2px，差異就從 0.17% 暴增到 62% 的畫面，工具直接拒絕輸出。

另外，如果這三張是重畫的（不是現在 repo 裡那張完稿的衍生版），`head` /
`torso` / `lower` 要改用**閉嘴那張**重新切一次，整組素材才會對得上。
