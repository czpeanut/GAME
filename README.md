...
`arm_l`/`arm_r`/`hair_front` 是美術另外提供的分層；`mouth` 是從三張嘴型完稿
截出來的（見下一節）；`eyes` 還沒有，所以眨眼目前是關著的。**沒提供的部件會
自動跳過**，可以一張一張補，程式完全不用改。
...
`eyes` / `mouth` 不必單獨畫一張透明底的五官。給**同一張立繪的不同版本**
——嘴閉合／半開／張開三張，或眼睛睜開／閉上兩張——`tools/cut-variants.py`
會找出「哪裡變了」，把那塊切成部件。它有兩種模式，差別只在你的三張圖是怎麼
產生的。

**模式一：三張圖逐像素對齊**（同一個檔案只換掉嘴巴那層再匯出）

```bash
python3 tools/cut-variants.py assets/characters/hero mouth \
    closed=raw/嘴閉.png half=raw/嘴半開.png open=raw/嘴張開.png
```

工具比對所有版本，取出有差異的區域，直接把那個方框切下來。這樣的接縫
**不是「幾乎看不出來」，而是數學上完全沒有接縫**：引擎把 `mouth` 疊在
`head` 上，而方框外每張圖都是一模一樣的像素，方框邊緣兩側的顏色本來就相同，
連羽化都不需要。實測「`head`（閉嘴版）＋ `mouth/open`」疊起來跟原本的張嘴
完稿 **RGB 完全相同**。

**模式二：三張圖是各自生成／各自重畫的**（`--window`）

這是實際遇到的情況。本角色的三張嘴型圖雖然構圖幾乎一樣，但線稿和陰影整張都
重跑過：任兩張之間有 **12～16% 的畫面**在變，連嘴巴周圍的下巴皮膚平均都差
18～22（滿值 255）。模式一在這裡完全沒用——切一個方框下來，換嘴型時整塊下巴
會跟著閃。位移補償也救不了，實測 ±10px 的平移沒有任何一個偏移量能降低差異。

所以模式二改成：**你指定一個小視窗，工具只在裡面找變化，切出一塊貼著嘴巴的
羽化遮罩**，而不是方框。

```bash
python3 tools/cut-variants.py assets/characters/hero mouth \
    --window 0.4627,0.1431,0.5572,0.1778 --seed 85 \
    --base assets/characters/hero/head/default.png \
    closed=raw/閉上.png half=raw/半開.png open=raw/全開.png
```

- `--window` 是搜尋範圍（畫面比例）。它同時也在**裁掉**視窗外的重繪雜訊，
  所以刻意畫小一點是對的；工具若說遮罩碰到視窗邊界，那是提醒不是錯誤。
- `--seed` 是「多大的差異才算是嘴巴、而不是重繪雜訊」。調高＝遮罩更貼嘴。
- `--base` 是身體其他部件的來源圖。**一定要給**：遮罩必須連 `base` 自己的
  嘴也蓋掉，否則張嘴時底下那張閉著的嘴會透出來。

結果：整張畫面只有 **404 個像素**會隨嘴型變化（0.14%），而且集中在嘴上。
三張嘴型檔各 **6 KB**。

> 順帶一提，`--window` 模式下三張圖的**畫布大小可以差一兩個像素**（這次
> `閉上.png` 是 403×720，另外兩張是 402×720）。工具會自己試每一種對齊方式，
> 取最吻合的那個，並把它選了哪個偏移印出來。

眼睛還沒有素材，所以眨眼目前仍是關著的；補上 `open.png` / `closed.png` 就會
自動開始眨眼，同一支工具、同樣的做法：

```bash
python3 tools/cut-variants.py assets/characters/hero eyes \
    --window <眼睛範圍> --base assets/characters/hero/head/default.png \
    open=raw/睜眼.png closed=raw/閉眼.png
```

### 部件圖不要留看不見的顏色

部件圖是「整張畫布、只有自己那塊不透明」，所以絕大多數像素是全透明的——但
PNG 還是會把那些像素的顏色老老實實存下來。把透明區的 RGB 清成黑色是看不出
差別的（瀏覽器用預乘 alpha 合成），檔案卻會小一個數量級：`head` 從 250 KB
變成 51 KB，嘴型從 250 KB 變成 6 KB。整個角色從 1.4 MB 降到 464 KB。

`split-parts.py` 和 `cut-variants.py` 都已經這樣做了。手邊如果有舊的部件圖，
重切一次就好。
